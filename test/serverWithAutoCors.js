...
test.beforeEach(async (t) => {
	t.context.domain1 = await Domain.create({ title: 'fqdn.example.com' })
        ...
        test.afterEach.always(
            async (t) => {await Domain
                              .findOneAndDelete({id : t.context.domain1.id})
...
const {headers : fqdnHeaders} =
    await fetch(url.href, {headers : {Host : 'fqdn.example.com'}})

t.is(fqdnHeaders.get('Access-Control-Allow-Origin'), 'fqdn.example.com')
t.is(fqdnHeaders.get('Access-Control-Allow-Methods'),
     'GET, POST, PATCH, OPTIONS')
t.is(fqdnHeaders.get('Access-Control-Allow-Headers'),
     'Content-Type, Authorization, Time-Zone')
t.is(fqdnHeaders.get('Access-Control-Allow-Credentials'), 'true')

const {headers : noFqdnHeaders} =
    await fetch(url.href, {headers : {Host : 'No fqdn'}})
