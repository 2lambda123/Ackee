...
test("return cors headers based on fully qualifed domain names", async (t) => {
  ...
  const { headers: fqdnHeaders } = await fetch(url.href, {
    headers: { Host: "fqdn.example.com" },
  });

  t.is(fqdnHeaders.get("Access-Control-Allow-Origin"), "fqdn.example.com");
  t.is(
    fqdnHeaders.get("Access-Control-Allow-Methods"),
    "GET, POST, PATCH, OPTIONS",
  );
  t.is(
    fqdnHeaders.get("Access-Control-Allow-Headers"),
    "Content-Type, Authorization, Time-Zone",
  );
  t.is(fqdnHeaders.get("Access-Control-Allow-Credentials"), "true");

  const { headers: noFqdnHeaders } = await fetch(url.href, {
    headers: { Host: "No fqdn" },
  });
